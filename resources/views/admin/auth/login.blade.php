...
<div class="flex flex-col min-h-screen bg-resort-background text-gray-900">
    ...
    <header class="w-full">
        <div class="flex items-center justify-center py-3">
            <img src="{{ asset('images/jara_logo.png') }}" alt="Jara's Palm Beach Resort Logo" class="h-80 w-auto mx-auto">
        </div>
    </header>
    ...
    <main class="flex-1 flex flex-col items-center justify-center px-4 py-3">
        <div class="w-full max-w-md bg-white border border-resort-accent rounded-resort shadow-resort p-8">
            <h2 class="text-2xl font-bold text-center font-crimson-pro text-resort-primary mb-8">Login</h2>
            ...
            <form method="POST" action="{{ route('admin.login') }}" class="space-y-6" id="login-form">
                ...
                <div>
                    <label for="email" class="block text-sm font-medium text-resort-gray-dark mb-1">Email</label>
                ...
                <div>
                    <label for="password" class="block text-sm font-medium text-resort-gray-dark mb-1">Password</label>
                    <div class="relative">
                ...
                @if (Route::has('admin.password.request'))
                    <a href="{{ route('admin.password.request') }}" class="block text-right text-resort-primary hover:underline text-sm mb-2">
                        Forgot Password?
                    </a>
                @endif
                ...
                <button type="submit" class="btn-resort-primary w-full py-3 font-semibold rounded-lg">
                    Log In
                </button>
            </form>
        </div>
...
<!-- Password encryption + form handling with reCAPTCHA -->
<script>
(function () {
    ...
    async function fetchPublicKey() {
        const res = await fetch('{{ route('admin.public_key') }}', { cache: 'no-store' });
    ...
    async function submitEncryptedLogin(pemKey) {
        const key = await importKey(pemKey);
        const pwd = passwordInput.value || '';
        const enc = new TextEncoder().encode(pwd);
        const cipher = await window.crypto.subtle.encrypt({ name: 'RSA-OAEP' }, key, enc);
        const b64 = arrayBufferToBase64(cipher);
    ...
    form.addEventListener('submit', async function (ev) {
        ...
        try {
            ...
            await submitEncryptedLogin(pem);
        } catch (err) {
